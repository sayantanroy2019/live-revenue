...
export const TotalOnlineRefundCardComponent = ({ amount = 0 }: { amount: number | undefined }) => {
  return (
    <Card className="dark:bg-[#1F1F1F] xl:min-w-[49%] 2xl:min-w-[49%] lg:min-w-full md:min-w-full sm:min-w-full max-sm:min-w-full flex flex-col items-center text-white  shadow-2xl border-none w-full max-w-md">
      ...
      <CardContent className="flex w-full flex-row items-start justify-between pb-2">
        <div className="flex items-center gap-3">
          <Image
            src={'/icons/live-revenue/total-online-refund-icon.png'}
          ...
          />
          <div>
            <CardTitle className="text-2xl font-bold">₹ {formatToIndianCurrency(amount)}</CardTitle>
            <p className="text-gray-400 text-sm uppercase">Total Online Refund</p>
          </div>
        </div>
